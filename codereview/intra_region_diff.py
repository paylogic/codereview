...
def RenderIntraLineDiff(blocks, line, tag, dbg_info=None, limit=80, indent=5,
    ...
    for curr_start, curr_len in blocks:
        ...
        res += FoldBlock(
            line, prev_start + prev_len, curr_start, limit, indent,
            tag, 'diff', tabsize, mark_tabs)
        res += FoldBlock(
            line, curr_start, curr_start + curr_len, limit, indent,
...
def IntraRegionDiff(old_lines, new_lines, diff_params):
    ...
    old_blocks, new_blocks, ratio = IntraLineDiff(
        old_line, new_line, diff_params)
...
def NormalizeBlocks(blocks, line):
    ...
    for curr_start, curr_len in blocks[1:]:
        # Note: nm_ is a prefix for non matching and m_ is a prefix for
        # matching.
