...
def PatchChunks(old_lines, chunks):
    ...
    for (old_i, old_j), _, old_chunk, new_chunk in chunks:
        ...
        #   yield ("error: old chunk mismatch", repr(old_lines[old_i:old_j]), repr(old_chunk))
        #   return
        # TODO(guido): ParsePatch knows the diff details, but throws the info
        # away
    ...
    for line in lines:
        ...
        if match:
            ...
            if tag in (" ", "-", "+"):
                ...
                # TODO(guido): need to check that no more lines follow for this
                # file
                if raw_chunk:
                ...
                if line and line.strip():
                    logging.warn("%s:%d: indecypherable input: %r",
                                 name, lineno, line)
