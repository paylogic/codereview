    ...
    def render(self, name, value, attrs=None):
        ...
        if models.Account.current_user_account is not None:
            ...
            data = {'name': name, 'url': reverse(account),
                    'multiple': 'true'}
            if self.attrs.get('multiple', True) is False:
                data['multiple'] = 'false'
            output += mark_safe(u'''
...
class SearchForm(forms.Form):
    ...
    modified_before = forms.DateTimeField(
        required=False, label='Modified before')
...
def respond(request, template, params=None):
    ...
    if request.user is not None:
        ...
        must_choose_nickname = not (
            account and account.user_has_selected_nickname())
        uploadpy_hint = getattr(account, 'uploadpy_hint', None)
...
def _can_view_issue(user, issue):
    if user is None:
        return False
    return user.has_perm('codereview.view_issue')
...
def _notify_issue(request, issue, message):
    ...
    for account in accounts.itervalues():
        logging.debug('email=%r,chat=%r', account.email,
                      account.notify_by_chat)
...
def patchset_required(func):
    ...
    def patchset_wrapper(request, patchset_id, *args, **kwds):
        patchset = models.PatchSet.get_by_id(
            int(patchset_id), parent=request.issue)
        if patchset is None:
            return HttpResponseNotFound(
                'No patch set exists with that id (%s)' %
                patchset_id)
...
def image_required(func):
    ...
    def image_wrapper(request, image_type, *args, **kwds):
        ...
        if image_type == "0":
            ...
            filename = request.patch.old_filename
        elif image_type == "1":
        ...
        if not content or not content.data:
            return HttpResponseRedirect(django_settings.STATIC_URL + "blank.jpg")
        request.content = open(
            os.path.join(BINARY_FILES_PATH, content.data)).read()
        request.content_type = mimetypes.guess_type(filename)[0]
        return func(request, *args, **kwds)
...
def _paginate_issues(page_url,
    ...
    if len(issues) > limit:
        del issues[limit:]
        params['next'] = _url(
            page_url, offset=offset + limit, **nav_parameters)
    ...
    if request.user is None:
        if IS_DEV:
            request.user = users.User(
                request.POST.get('user', 'test@example.com'))
        else:
    ...
    if form.is_valid():
        ...
        if issue_id:
            ...
            if issue is None:
                ...
                form.errors['issue'] = [
                    'Base files upload required for that issue.']
                issue = None
            else:
                if request.user != issue.owner:
                    form.errors[
                        'user'] = ['You (%s) don\'t own this issue (%s)' %
                                   (request.user, issue_id)]
                    issue = None
                else:
                    patchset = _add_patchset_from_form(
                        request, issue, form, 'subject',
    ...
    if issue is None:
        ...
        if (form.cleaned_data.get('content_upload') or
            ...
            if form.cleaned_data.get('content_upload'):
                # Extend the response: additional lines are the expected
                # filenames.
                ...
                for patch in patches:
                    content = None
                    # Check if the base file is already uploaded in another
                    # patchset.
                    if (patch.filename in base_hashes and
                        ...
                        patch.is_binary = existing_patches[
                            patch.filename].is_binary
                    if not content:
                        content = models.Content(
                            is_uploaded=True, parent=patch)
                ...
                for patch, content_entity in zip(patches, content_entities):
                    ...
                    if content_entity not in new_content_entities:
                        ...
                        # be uploaded.  We mark this by prepending 'nobase' to
                        # the id.
...
def _make_new(request, form):
    ...
    def txn():
        issue = models.Issue(subject=form.cleaned_data['subject'],
                             ...
                             base=base,
                             repo_guid=form.cleaned_data.get(
                                 'repo_guid', None),
                             reviewers=reviewers,
                             cc=cc,
        ...
        patchset = models.PatchSet(
            issue=issue, data=data, url=url, parent=issue)
        patchset.put()
...
def _get_data_url(form):
    ...
    if not (data or url or separate_patches):
        form.errors['data'] = [
            'You must specify a URL or upload a file (< 1 MB).']
        return None
    if data and url:
    ...
    if data is not None:
        ...
        if fetch_result.status_code != 200:
            form.errors['url'] = ['HTTP status code %s' %
...
def _add_patchset_from_form(request, issue, form, message_key='message',
    ...
    # TODO(guido): use a transaction like in _make_new(); may be share more
    # code?
    if form.is_valid():
        data_url = _get_data_url(form)
    if not form.is_valid():
        return None
    if request.user != issue.owner:
        # This check is done at each call site but check again as a safety
        # measure.
        return None
    data, url, separate_patches = data_url
    message = form.cleaned_data[message_key]
    patchset = models.PatchSet(
        issue=issue, message=message, data=data, url=url,
        parent=issue)
    patchset.put()
...
def _calculate_delta(patch, patchset_id, patchsets):
    ...
    for other in patchsets:
        ...
        if other.data or other.parsed_patches:
            ...
            for filename, old_filename, text, is_binary in other.parsed_patches:
...
def _get_patchset_info(request, patchset_id):
    ...
    if request.user:
        ...
        drafts = []
    comments = list(
        models.Comment.gql('WHERE ANCESTOR IS :1 AND draft = FALSE',
                           issue))
    ...
    for i, patchset in enumerate(patchsets):
        ...
        file_set_prev = set() if i == 0 else set(
            [patch.filename for patch in patchsets[i - 1].patch_set])
        if patchset_id == patchset.key().id():
            ...
            try:
                ...
                for patch in patchset.patches:
                    pkey = patch.key()
                    patch._num_comments = sum(
                        c.parent_key() == pkey for c in comments)
                    patch._num_drafts = sum(
                        c.parent_key() == pkey for c in drafts)
                    if not patch.delta_calculated:
                        if attempt > 2:
                            ...
                            # memory so don't ever call patchset.put() after
                            # calling it.
                            patch.delta = _calculate_delta(
                                patch, patchset_id, patchsets)
                ...
                logging.exception(
                    'DeadlineExceededError in _get_patchset_info')
                if attempt > 2:
...
def show(request, form=None):
    ...
    return respond(request, 'issue.html',
                   {'issue': issue, 'patchsets': patchsets,
                    'issue_messages': messages, 'form': form,
...
def patchset(request):
    ...
    patchset = request.patchset
    issue, patchsets, response = _get_patchset_info(
...
def download(request):
    ...
    if user_agent and 'MSIE' in user_agent:
        # Add 256+ bytes of padding to prevent XSS attacks on Internet
        # Explorer.
        padding = ('=' * 67 + '\n') * 4
...
def image(request):
    ...
    return HttpResponse(request.content, content_type=request.content_type)
...
def _get_diff_table_rows(request, patch, context, column_width):
    ...
    # Possible engine.FetchErrors are handled in diff() and
    # diff_skipped_lines().
...
def _get_diff2_data(request, ps_left_id, ps_right_id, patch_id, context,
    ...
    ps_right = models.PatchSet.get_by_id(
        int(ps_right_id), parent=request.issue)
    if ps_right is None:
        return HttpResponseNotFound('No patch set exists with that id (%s)' %
                                    ps_right_id)
...
def _inline_draft(request):
    ...
    comments = list(
        c for c in query if not c.draft or c.author == request.user)
...
def _get_affected_files(issue, full_diff=False):
    ...
    if len(patchsets):
        ...
        for patch in patchset.patch_set.order('filename'):
            ...
            # No point in loading patches if the patchset is too large for
            # email.
            ...
            if patch.no_base_file:
                ...
                try:
                    if c.left:
                        ...
                        new_lines = patch.get_patched_content().text.splitlines(
                            True)
                        linecache[last_key] = new_lines
                except engine.FetchError:
                    linecache[last_key] = patching.ParsePatchToLines(
                        patch.lines)
...
def _make_message(request, issue, message, comments=None, send_mail=False,
    ...
    template, context = _get_mail_template(
        request, issue, full_diff=attach_patch)
    ...
    if send_mail:
        ...
        url = request.build_absolute_uri(
            reverse(show, args=[issue.key().id()]))
        ...
        if patch:
            send_args[
                'attachments'] = [('issue_%s_patch.diff' % issue.key().id(),
                                   patch)]
...
@permission_required('codereview.change_comment')
@issue_required
...
def search(request):
    ...
    if keys_only:
        ...
        filtered_results = [
            i for i in results if _can_view_issue(request.user, i)]
    data = {
...
def _user_popup(request):
    ...
    if popup_html is None:
        ...
        popup_html = render_to_response('user_popup.html',
                                        ...
                                        context_instance=RequestContext(request))
        # Use time expired cache because the number of issues will change over
        # time
...
def _process_incoming_mail(raw_message, recipients):
    ...
    if sender.lower() not in all_emails:
        ...
        account = query.get()
        if account is not None:
            # e.g. account.email is CamelCase
            issue.reviewers.append(account.email)
        else:
