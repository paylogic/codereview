...
def test_gatekeepers_approve(app, issue, patch, monkeypatch, patchset_revision):
    """Test gatekeeper approval form."""
    with mock.patch('fogbugz.FogBugz', mock.DEFAULT) as mocked_fogbugz:
        mocked_value = mock.Mock()
        mocked_value.string = 'Some'

        setattr(mocked_fogbugz().search(), settings.FOGBUGZ_CI_PROJECT_FIELD_ID, mocked_value)
...
@pytest.mark.parametrize('user_permissions', (['view_issue', 'approve_patchset'],))
def test_gatekeepers_approve_no_ci_project(app, issue, patch):
    """Test gatekeeper approval form."""
    with mock.patch('fogbugz.FogBugz', mock.DEFAULT) as mocked_fogbugz:
        mocked_value = mock.Mock()
        mocked_value.string = '--'
