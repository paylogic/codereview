...
class AbstractRpcServer(object):
    ...
    def __init__(
        ...
        if self.host_override:
            logging.debug(
                "Server: %s; Host: %s", self.host, self.host_override)
        else:
            logging.debug("Server: %s", self.host)
    ...
    def _GetAuthToken(self, email, password):
        ...
        account_type = "HOSTED"
        if self.host.endswith("appspot.com"):
            account_type = "GOOGLE"
    ...
    def _GetAuthCookie(self, auth_token):
        ...
        if (response.code != 302 or
            ...
            raise urllib2.HTTPError(
                req.get_full_url(), response.code, response.msg,
...
def RunShellWithReturnCode(command, print_output=False,
    ...
    if errout and not ignore_stderr:
        output = output + errout
    return output, p.returncode


def RunShell(command, silent_ok=False, universal_newlines=True,
             print_output=False, env=os.environ, check_returncode=True, cwd=None,
             ignore_stderr=False):
    data, retcode = RunShellWithReturnCode(command, print_output,
                                           universal_newlines, env, cwd=cwd,
                                           ignore_stderr=ignore_stderr)
    if retcode and check_returncode:
        ErrorExit("Got error status from %r:\n%s (%s)" %
                  (' '.join(command), data, retcode))
    if not silent_ok and not data:
        ErrorExit("No output from %r" % ' '.join(command))
    return data
...
class VersionControlSystem(object):
    ...
    def __init__(self, options, repo_dir):
        ...
        if not os.path.exists(self.repo_dir):
            # use initial repo dir because it can be remote
    ...
    def UploadBaseFiles(self, issue, rpc_server, patch_list, patchset, options,
        ...
        for filename in patches.keys():
            ...
            if base_content is not None:
                UploadFile(
                    filename, file_id, base_content, is_binary, status, True)
            if new_content is not None:
                UploadFile(
                    filename, file_id, new_content, is_binary, status, False)
    ...
    def _GuessBase(self, required):
        ...
        for line in info.splitlines():
            ...
            if len(words) == 2 and words[0] == "URL:":
                ...
                if username:
                    logging.debug("Removed username from base URL")
                if netloc.endswith("svn.python.org"):
                    ...
                    logging.debug("Guessed Python base = %s", base)
                elif netloc.endswith("svn.collab.net"):
                    if path.startswith("/repos/"):
                        path = path[6:]
                    base = "http://svn.collab.net/viewvc/*checkout*%s/" % path
                    logging.debug("Guessed CollabNet base = %s", base)
                elif netloc.endswith(".googlecode.com"):
                    path = path + "/"
                    base = urlparse.urlunparse(("http", netloc, path, params,
                                                query, fragment))
                    logging.debug("Guessed Google Code base = %s", base)
                else:
                    path = path + "/"
                    base = urlparse.urlunparse((scheme, netloc, path, params,
                                                query, fragment))
                    logging.debug("Guessed base = %s", base)
                return base
        if required:
        ...
        if status[0] == "A" and status[3] != "+":
            ...
            if get_base:
                ...
                if self.rev_start:
                    ...
                    # the full URL with "@REV" appended instead of using "-r"
                    # option.
                    url = "%s/%s@%s" % (
                ...
                if not is_binary:
                    ...
                    if keywords and not returncode:
                        base_content = self._CollapseKeywords(
...
class GitVCS(VersionControlSystem):
    ...
    def Pull(self, source, *args):
        ...
        RunShell(['git', 'remote', 'add', 'source', source.repo_dir],
...
class MercurialVCS(VersionControlSystem):
    ...
    def __init__(self, options, repo_dir):
        super(MercurialVCS, self).__init__(options, repo_dir)
        if not self.options.revision:
            self.base_rev = RunShell(
                ["hg", "-R", self.repo_dir, "parent", "-q"]).split(':')[1].strip()
    ...
    def Clone(self, dest):
        RunShell([
            ...
            self.repo_dir, dest],
            silent_ok=True)
        target = MercurialVCS(self.options, dest)
    ...
    def GetBaseFile(self, filename):
        ...
        out = RunShell(["hg", '-R', self.repo_dir,
                        "status", "-C", "--rev", self.base_rev, relpath])
        out = out.splitlines()
        ...
        if is_binary and base_content:
            ...
            base_content = RunShell(["hg", '-R', self.repo_dir,
                                     "cat", "-r", base_rev, oldrelpath],
                                    silent_ok=True, universal_newlines=False)
        if not is_binary or not self.IsImage(relpath):
            new_content = None
        return base_content, new_content, is_binary, status

    def GetBaseFile2(self, filename):
        if filename.startswith(self.repo_dir.rstrip('/') + '/'):
            filename = filename[len(self.repo_dir.rstrip('/') + '/'):]
        try:
            content = RunShell(['hg', '-R', self.repo_dir,
                                'cat', '-r', self.base_rev, filename],
                               silent_ok=True, check_returncode=False, cwd=self.repo_dir)
        except:
            content = ''
        if ': no such file in rev ' in content and content.count('\n') < 2:
            content = ''
        return content
...
#    filename = os.path.join(self.repo_dir, filename)
    ...
    def get_cwd(self):
        """Get cwd."""
        cwd = os.path.join(self.repo_dir, self.base_rev)
        if not os.path.isdir(cwd):
            # repo dir includes branch
            cwd = self.repo_dir
        return cwd
...
def GuessVCSName():
    ...
    # bzr info will return an exit code of 0 if you're in a branch
    try:
        out, returncode = RunShellWithReturnCode(["bzr", "root"])
        if returncode == 0:  # if return code isn't 0 this isn't a branch
            return (VCS_BAZAAR, out.strip())
    except OSError, (errno, message):
        if errno != 2:  # ENOENT -- they don't have hg installed.
            raise
    ...
    if os.path.isdir('.svn'):
        logging.debug("Guessed VCS = Subversion")
...
def GuessVCS(options, *args):
    ...
    if vcs == VCS_MERCURIAL:
        return MercurialVCS(options, *args)
    elif vcs == VCS_SUBVERSION:
        return SubversionVCS(options, *args)
    elif vcs == VCS_GIT:
        return GitVCS(options, *args)
    elif vcs == VCS_BAZAAR:
        return BazaarVCS(options, *args)
...
def LoadSubversionAutoProperties():
    ...
    """
    # Todo(hayato): Windows users might use different path for configuration
    # file.
...
def RealMain(argv, data=None):
    ...
    if not base and options.download_base:
        ...
        logging.debug("Enabled upload of base file")
    ...
    if not options.download_base:
        vcs.UploadBaseFiles(
            issue, rpc_server, patches, patchset, options, files)
